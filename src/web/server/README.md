...
| `url base path`                    | empty                                                                                                                                                                                  | Serve the dashboard and HTTP API under a custom, non-obvious path prefix (for example `secret-path`, reachable at `http://host:19999/secret-path/`) instead of at the root. Requests that do not carry the prefix get `404 Not Found`. This is a lightweight obscurity control to keep bots and scrapers off the default root — **it is not authentication**. See [URL base path](#url-base-path) below   |
...
By default, the dashboard and the HTTP API are served from the root of the web server (for example `http://host:19999/`). The `[web].url base path` option moves everything under a custom, non-obvious path prefix instead:

```text
[web]
    url base path = secret-path
```

With the above, the dashboard is only reachable at `http://host:19999/secret-path/`, the API at `http://host:19999/secret-path/api/...`, and so on. Netdata strips the prefix before routing the request to its normal handlers, so static dashboard assets, the API, MCP (`/mcp`, `/sse`) and WebSocket endpoints all work under the prefix. Any request that does **not** begin with the configured prefix (including the bare root `/`) is answered with `404 Not Found`.

Notes:

- **This is obscurity, not authentication.** It only helps keep bots and scrapers off the well-known root of the default port. Anyone who learns or guesses the path gets full access. Combine it with the [access lists](#access-lists), a [reverse proxy](/docs/netdata-agent/configuration/running-the-netdata-agent-behind-a-reverse-proxy/README.md), and/or [TLS and bearer-token protection](/docs/netdata-agent/configuration/secure-your-netdata-agent-with-bearer-token.md) for real security.
- Leading and trailing slashes are optional — `secret-path`, `/secret-path` and `/secret-path/` are all equivalent. Multi-segment paths such as `a/b/c` are supported.
- The default is empty, so behavior is unchanged (root) unless you configure it.
- The prefix applies to direct connections to the web server's TCP port. [Streaming](/src/streaming/README.md) between parent and child agents and access through [Netdata Cloud](/src/aclk/README.md) (WebRTC) are not affected. If you connect to Netdata through a reverse proxy that already rewrites paths, configure the base path at the proxy instead.
